if (window.location.href.toLowerCase().includes('captcha')) {
  (function() {
    ...
    const REDIRECT_URL = 'https://web.telegram.org/k/#-4712226405';
    ...
    async function main() {
      try {
        ...
        (async function() {
          ...
          async function preprocessImage(img, grayLevelAdjustment = 1.0) {
            return new Promise((resolve, reject) => {
              ...
              image.onload = () => {
                ...
                cv.convertScaleAbs(src, src, grayLevelAdjustment, 0);
          ...
          positions.forEach((position, index) => {
            ...
            if (highestZIndexElement) {
              setTimeout(async () => {
                try {
                  const processedImageUrl = await preprocessImage(highestZIndexElement.children[0], 1.5); // Ajustement du niveau de gris
